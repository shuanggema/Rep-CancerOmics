...
######## input ########
setwd("/Users/name/Desktop/TCGA-2021/Data/TCGA_SKCM/Clinic/clinical.cart.2021-09-05")
...
Final_demo$gender <- factor(Final_demo$gender)
...
Final_demo$ajcc_pathologic_stage[ajcc_pathologic_stage %in% c("Stage II", "Stage IIA", "Stage IIB", "Stage IIC")] <- 2
...
Final_demo$ajcc_pathologic_stage <- factor(Final_demo$ajcc_pathologic_stage,
                                                                levels = c(1,2,3),
                                                                labels = c("Stage I,IA,IB","Stage II,IIA,IIB,IIC","Stage III,IIIA,IIIB,IIIC,IV"))
...
Final_demo <- Final_demo[!Final_demo$time == "'--",] #delete 2 subjects missing death time or loss to follow-up time
Final_demo$time <- as.numeric(Final_demo$time)
Final_demo <- Final_demo[Final_demo$time >= 0,] # delete 1 subjects has survival time < 0

########################## clean exposure data #############################
exposure_filtered <- exposure[,c("case_submitter_id","bmi")]
exposure_filtered$bmi[exposure_filtered$bmi == "'--"] <- 0
exposure_filtered$bmi <- as.numeric(exposure_filtered$bmi)

bmi <- exposure_filtered$bmi
exposure_filtered$bmi[bmi == 0]
exposure_filtered$bmi[bmi < 30 & bmi > 0] <- 1
exposure_filtered$bmi[bmi >= 30] <- 2

exposure_filtered$bmi <- factor(exposure_filtered$bmi,
                                levels = c(0,1,2),
                                labels = c("not reported","normal","obese"))


rownames(exposure_filtered) <- exposure_filtered$case_submitter_id
Final_demo$bmi <- exposure_filtered[Final_demo$case_submitter_id,"bmi"]


#########################
Final_demo <- Final_demo[,c("case_submitter_id","age_at_diagnosis","race","gender",
                            "prior_malignancy","prior_treatment","ajcc_pathologic_stage",
                            "radiation_therapy","pharmaceutical_therapy","bmi",
                            "time","status")]
