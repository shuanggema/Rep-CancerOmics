...
######## input ########
data_before_sample_filter <- read.csv("/Users/name/Desktop/TCGA-2021/Data/TCGA_SKCM/Gene/GeneID_EnsebmleID_barcode_log(TPM+1)_data_protein.csv",
                                      row.names = 1)
...
# if different samples for one patient, retain 01 (primary solid tumor) instead of 06 (metastatic)
delete_barcode_list <- c("TCGA.ER.A19T.06A.11R.A18U.07", "TCGA.ER.A2NF.06A.11R.A18T.07")

retain_barcode_list <- retain_barcode_list[!retain_barcode_list %in% delete_barcode_list]

############# finally we have 468 subjects ################
Final_gene_data <- data.frame(data_before_sample_filter[,1:3],data_before_sample_filter[,retain_barcode_list])
write.csv(Final_gene_data, "/Users/name/Desktop/TCGA-2021/Data/TCGA_SKCM/Gene/Final_gene_data.csv")
